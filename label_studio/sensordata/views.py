    ...
    if request.method =='POST':
        ...
        if sensordataform.is_valid():
            ...
            match sensortype.sensortype:
                case 'I':
                    parse_IMU(request=request, file_path=file_path,sensor_type_id=sensortype.id,name=name)
                case 'C':
...
def parse_IMU(request, file_path, sensor_type_id, name):
    ...
    upload_sensor_data(request, file_path,project_id=1)
