...
class DeploymentForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'begin_datetime': forms.DateTimeInput(
                attrs={'style': 'display: block; margin-bottom: 0.5em;', 'type': 'datetime-local', 'step': 1}),
        ...
        for sensor in sensors:
            for deployment in deployments:
                if sensor in deployment.sensor.all():
                    if begin_datetime <= deployment.begin_datetime <= end_datetime:
                        overlapping_deployment_begin.append(str(deployment))
                        overlap_begin = True
                    if begin_datetime <= deployment.end_datetime <= end_datetime:
